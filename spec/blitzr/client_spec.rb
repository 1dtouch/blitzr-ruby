...
describe Blitzr::Client do
  ...
  describe 'initialization' do
    ...
    it 'defaults :api_endpoint to production API' do
      ...
      expect(subject.api_endpoint).to eq('http://api.blitzr.com/')
    end
  end
  ...
  describe 'authentication' do
    it 'raises an error if there is no api key provided' do
      ...
      }.to raise_error(Blitzr::Error, 'An API key is required for all API requests.')
    end
  end
  ...
  describe '#execute' do
    ...
    it 'raises RequestError in case of error' do
      ...
      }.to raise_error(Blitzr::RequestError, '500')
    end
  end
  ...
  describe '#request' do
    ...
    it 'performs a request' do
      ...
      expect(WebMock)
        .to have_requested(:get, 'http://api.blitzr.com/foo')
        .with(
          headers: {
            'Accept' => 'application/json',
            'User-Agent' => "blitzr-ruby/#{Blitzr::VERSION}"
          },
          query: { 
            key:     'key',
            _format: 'json' 
